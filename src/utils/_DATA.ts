...
export function _saveLikeToggle(hasLined: any) {
  const {id, hasLiked, authedUser} = hasLined;
  return new Promise((res, rej) => {
    setTimeout(() => {
      tweets = {
        ...tweets,
        [ id ]: {
          ...tweets[ id ],
          likes: hasLiked === true
            ? tweets[ id ].likes.filter((uid: string) => uid !== authedUser)
...
function formatTweet(tweet: any) {
  const {author, text, replyingTo = null} = tweet;
  return {
    author,
    id: generateUID(),
    likes: [],
...
export function _saveTweet(tweet: any) {
  const {text, author, replyingTo} = tweet;
  return new Promise((res, rej) => {
